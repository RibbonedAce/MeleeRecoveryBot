...
import Chains
from Chains import FalconDive, AirDodge, RaptorBoost
...
class Recover(Tactic):
    ...
    def step_internal(self, game_state, smashbot_state, opponent_state):
        if Chains.EdgeDash.should_use(self._propagate):
            self.pick_chain(Chains.EdgeDash)
            return
        ...
        if Chains.DriftIn.should_use(self._propagate) and \
        ...
        if Chains.FastFall.should_use(self._propagate):
            self.pick_chain(Chains.FastFall)
            return

        # If we are currently moving away from the stage, DI in
        if Chains.DriftIn.should_use(self._propagate) and Recover.__can_hold_drift(smashbot_state, opponent_state, stage_edge):
            self.chain = None
            self.pick_chain(Chains.DriftIn)
            return
        ...
        if Chains.RaptorBoost.should_use(self._propagate) and self.recover_mode == RECOVER_MODE.SECONDARY and \
                RaptorBoost.TRAJECTORY.get_extra_distance(smashbot_state, opponent_state, target, self.ledge, 0) > 0:
            self.chain = None
            self.pick_chain(Chains.RaptorBoost, [target, self.fade_back_mode, self.ledge])
            return
        ...
        if Chains.FalconDive.should_use(self._propagate) and \
                (smashbot_state.speed_y_self <= 0 or wall_teching) and self.time_to_recover:
            self.chain = None
            self.pick_chain(Chains.FalconDive, [target, self.fade_back_mode, self.ledge])
            return

        # If we can do a Falcon Kick to get back easier, then do so
        if Chains.FalconKick.should_use(self._propagate):
            self.pick_chain(Chains.FalconKick)
            return
