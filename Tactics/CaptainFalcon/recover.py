...
import Chains
from Chains import AirDodge
from Chains.CaptainFalcon import FalconDive, RaptorBoost
...
class Recover(Tactic):
    ...
    def step_internal(self, game_state, smashbot_state, opponent_state):
        ...
        if Chains.CaptainFalcon.RaptorBoost.should_use(self._propagate) and self.recover_mode == RECOVER_MODE.SECONDARY and \
            ...
            self.chain = None
            self.pick_chain(Chains.CaptainFalcon.RaptorBoost, [target, self.fade_back_mode, self.ledge])
            return
        ...
        if Chains.CaptainFalcon.FalconDive.should_use(self._propagate) and \
            ...
            self.chain = None
            self.pick_chain(Chains.CaptainFalcon.FalconDive, [target, self.fade_back_mode, self.ledge])
            return

        # If we can do a Falcon Kick to get back easier, then do so
        if Chains.CaptainFalcon.FalconKick.should_use(self._propagate):
            self.pick_chain(Chains.CaptainFalcon.FalconKick)
            return
